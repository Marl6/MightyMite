...
class User{
	...
	protected static $tbl_name = "tblcustomer";
	function db_fields(){
		global $mydb;
	...
	function findCURCustomer($username= "", $EMPID=""){
			global $mydb;
			$mydb->setQuery("SELECT * FROM ".self::$tbl_name." Where `USERNAME`= '{$username}' AND `EMPID`= '{$EMPID}' LIMIT 1");
	...
	function find_all_user($name=""){
	 ...
	 static function AuthenticateUser($email="", $h_upass=""){
		global $mydb;
		$mydb->setQuery("SELECT * FROM `tblcustomer` WHERE `USERNAME`='" . $email . "' and `PASSWRD`='" . $h_upass ."' LIMIT 1");
		$cur = $mydb->executeQuery();
		$row_count = $mydb->num_rows($cur);//get the number of count
		 if ($row_count == 1){
